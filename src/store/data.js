import formalShirt from '../assets/formalShirt.webp'
import formalShirtD from '../assets/formalShirtD.webp'
import saree from '../assets/women2.webp'
import sareeD from '../assets/women2D.webp'
import formalPant from '../assets/menFormalPant.jpg'
import formalPantD from '../assets/menFormalPantD.webp'
import tshirt from '../assets/men3.webp'
import tshirtD from '../assets/men3D.webp'
import dress from '../assets/womenDress.webp'
import dressD from '../assets/womenDressD.webp'
import lower from '../assets/menLower.webp'
import lowerD from '../assets/menLowerD.webp'
import leggis from '../assets/womenHeels.webp'
import leggisD from '../assets/womenHeelsD.webp'
import womenSkirts from '../assets/womenSkirtts.webp'
import kurti from '../assets/kurti.webp'
import kurtiD from '../assets/kurtiD.webp'
import hoodie from '../assets/hoodie.webp'
import hoodieD from '../assets/hoodieD.webp'
import KidsDresses from '../assets/KidsDresses.webp'
import KidsDressesD from '../assets/KidsDressesD.webp'
import gown from '../assets/gown.webp'
import gownD from '../assets/gownD.webp'
import denimMen from '../assets/denimMen.webp'
import denimMenD from '../assets/denimMenD.webp'
import jacketMen from '../assets/jacketMen.webp'
import jacketMenD from '../assets/jacketMenD.webp'
import jeans from '../assets/jeans.webp'
import jeansD from '../assets/jeansD.webp'
import chain from '../assets/chain.webp'
import chainD from '../assets/chainD.webp'
import palazoo from '../assets/plazo.webp'
import palazooD from '../assets/plazoD.webp'
import topWomen from '../assets/topWomen.webp'
import topWomenD from '../assets/topWomenD.webp'
import kneeDress from '../assets/kneeDress.webp'
import kneeDressD from '../assets/kneeDressD.webp'
import blazer from '../assets/blazer.webp'
import blazerD from '../assets/blazerD.webp'
import kurta from '../assets/kurta.webp'
import kurtaD from '../assets/kurtaD.webp'
import menShirt from '../assets/menShirt.avif'
import menTshirt from '../assets/menTshirt.webp'
import menJeans from '../assets/menStraightJeans.jpeg'
import menKurta from '../assets/menKurta.jpg'
import menWaist from '../assets/mwnWaistCoat.webp'
import menShirts from '../assets/menShirt.jpg'
import rcbJearsy from '../assets/MenJersey.jpg'
import menLower from '../assets/MenLower.jpeg'
import menSneaker from '../assets/MenWhiteSneakers.jpg'
import menBoot from '../assets/MenBoot.avif'
import womenLehenga from '../assets/WomenLehenga.jpg'
import womenKurti from '../assets/WomenDupata.webp'
import womenSarree from '../assets/WomenSarrree.webp'
import womenJewel from '../assets/womenJewel.webp'
import womenKurtaSharara from '../assets/WomenKurtaSharara.jpg'
import womenBlouse2 from '../assets/womenBlouse2.webp'
import womenJeans from '../assets/WomenJeans2.webp'
import womenRRSa from '../assets/WomenRRSa.webp'
import kidsTshirt from '../assets/kidsTshirts.webp'
import kidsJersey from '../assets/KidsDhoniJersey.webp'
import kidsOverSizedTshirt from '../assets/kidsOverSizeTshirt.webp'
import kidsEthnic from '../assets/kidsEthenic.webp'
import kidsCasual from '../assets/kidsCasual.webp'
import kidsParty from '../assets/kidsPartyWear.webp'
import kidsOtherParty from '../assets/kidsPartywearBoys.webp'
import kidsCrocks from '../assets/KidsCrocks.webp'
import kidsFootwear from '../assets/kidsSandles.webp'
import kidsToys from '../assets/kidsTOys.webp'
import womenTrouser from '../assets/WomenLooseTrousers.webp'

export const data = [
  {
    id: 41,
    title: "Kurti",
    rating: 4.5,
    image: {
      mobile: kurti,
      desktop: kurtiD
    },
    category: "Women",
    price: 22,
    discountPrice: 35,
    description: "Soft rayon fabric kurti with elegant ethnic prints. Perfect for office wear, daily use, and festive styling.",
  },
  {
    id: 42,
    title: "Formal Shirt",
    rating: 4.2,
    image: {
      mobile: formalShirt,
      desktop: formalShirtD
    },
    description: "Premium cotton formal shirt with sharp fitting and breathable fabric for office and meetings.",
    category: 'Men',
    price: 28,
    discountPrice: 42
  },
  {
    id: 43,
    title: 'Saree',
    rating: 4.3,
    image: {
      mobile: saree,
      desktop: sareeD
    },
    description: "Lightweight saree with rich borders and smooth texture for graceful traditional looks.",
    category: 'Women',
    price: 30,
    discountPrice: 48
  },
  {
    id: 44,
    title: 'Formal Pant',
    rating: 4.3,
    image: {
      mobile: formalPant,
      desktop: formalPantD
    },
    description: "Slim-fit formal trousers with stretch comfort for long office hours and professional use.",
    category: 'Men',
    price: 25,
    discountPrice: 32
  },
  {
    id: 45,
    title: 'T-shirt',
    rating: 4.6,
    image: {
      mobile: tshirt,
      desktop: tshirtD
    },
    description: "Trendy casual t-shirt made from soft cotton fabric for daily wear and casual outings.",
    category: 'Men',
    price: 24,
    discountPrice: 50
  },
  {
    id: 46,
    title: 'Women Dress',
    rating: 4.3,
    image: {
      mobile: dress,
      desktop: dressD
    },
    description: "Stylish women dress with smooth fall and modern fitting for parties and casual events.",
    category: 'Women',
    price: 29,
    discountPrice: 55
  },
  {
    id: 47,
    title: 'Lower',
    rating: 4.1,
    image: {
      mobile: lower,
      desktop: lowerD
    },
    description: "Comfort fit casual lower for jogging, gym workouts, and daily home wear.",
    category: 'Men',
    price: 20,
    discountPrice: 45
  },
  {
    id: 48,
    title: 'Heels',
    rating: 4.3,
    image: {
      mobile: leggis,
      desktop: leggisD
    },
    description: "Stretchable leggings with smooth fabric for perfect comfort and daily flexibility.",
    category: 'Women',
    price: 18,
    discountPrice: 45
  },
  {
    id: 49,
    title: 'Hoodie',
    rating: 4.5,
    image: {
      mobile: hoodie,
      desktop: hoodieD
    },
    description: "Warm winter hoodie with premium fleece lining for street style and winter protection.",
    category: 'Men',
    price: 35,
    discountPrice: 54
  },
  {
    id: 50,
    title: 'Tunic Pant Set',
    rating: 4.4,
    image: {
      mobile: KidsDresses,
      desktop: KidsDressesD
    },
    description: "Ultra-soft breathable women innerwear designed for daily comfort and hygiene.",
    category: 'Women',
    price: 15,
    discountPrice: 30
  }
]


export const collectionData = [
  {
    id: 31,
    title: 'Denim',
    description: "Premium denim jeans with strong stitching and modern street-style fit.",
    rating: 4.5,
    price: 35,
    image: {
      mobile: denimMen,
      desktop: denimMenD
    },
    category: 'Men',
    discountPrice: 45
  },
  {
    id: 32,
    title: 'Men Jacket',
    description: "Windproof winter jacket with padded heating layer for cold protection.",
    rating: 4.2,
    price: 32,
    image: {
      mobile: jacketMen,
      desktop: jacketMenD
    },
    category: 'Men',
    discountPrice: 44
  },
  {
    id: 33,
    title: 'Jeans',
    description: "Slim-fit men jeans with flexible fabric for daily and casual wear.",
    rating: 4.6,
    price: 34,
    image: {
      mobile: jeans,
      desktop: jeansD
    },
    category: 'Men',
    discountPrice: 50
  },
  {
    id: 34,
    title: 'Chain',
    description: "Stylish polished chain for trendy casual and party fashion.",
    rating: 4.2,
    price: 18,
    image: {
      mobile: chain,
      desktop: chainD
    },
    category: 'Men',
    discountPrice: 28
  },
  {
    id: 35,
    title: 'Plazoo',
    description: "Loose-fit palazzo pants with breathable fabric for summer fashion.",
    rating: 4.4,
    price: 25,
    image: {
      mobile: palazoo,
      desktop: palazooD
    },
    category: 'Women',
    discountPrice: 32
  },
  {
    id: 36,
    title: 'Top',
    description: "Trendy crop top with smooth fabric and party-ready design.",
    rating: 4.0,
    price: 21,
    image: {
      mobile: topWomen,
      desktop: topWomenD
    },
    category: 'Women',
    discountPrice: 30
  },
  {
    id: 37,
    title: 'Girl Dress',
    description: "Elegant knee-length dress for parties, dinner dates, and events.",
    rating: 4.3,
    price: 29,
    image: {
      mobile: kneeDress,
      desktop: kneeDressD
    },
    category: 'Women',
    discountPrice: 40
  },
  {
    id: 38,
    title: 'Gown',
    description: "Designer evening gown with luxurious fabric and royal finish.",
    rating: 4.1,
    price: 39,
    image: {
      mobile: gown,
      desktop: gownD
    },
    category: 'Women',
    discountPrice: 59
  },
  {
    id: 39,
    title: 'Blazer',
    description: "Formal blazer for office and events with sharp tailoring.",
    rating: 4.5,
    price: 31,
    image: {
      mobile: blazer,
      desktop: blazerD
    },
    category: 'Men',
    discountPrice: 40
  },
  {
    id: 40,
    title: 'Kurta',
    description: "Classic traditional kurta with lightweight cotton fabric.",
    rating: 4.8,
    price: 30,
    image: {
      mobile: kurta,
      desktop: kurtaD
    },
    category: 'Men',
    discountPrice: 35
  }
]


export const menData = [
  {
    id: 21,
    title: 'Party Wear',
    description: 'Stylish party wear shirt crafted with premium fabric for evening events, celebrations, and special occasions.',
    rating: 4.6,
    price: 24,
    discountPrice: 36,
    image: menShirt,
    category: 'Man',
  },
  {
    id: 22,
    title: 'T-shirt',
    description: 'Comfortable cotton t-shirt designed for everyday casual wear with a relaxed and breathable fit.',
    rating: 4.3,
    price: 18,
    discountPrice: 29,
    image: menTshirt,
    category: 'Man',
  },
  {
    id: 23,
    title: 'Jeans',
    description: 'Classic slim-fit jeans built for durability and style, perfect for daily wear and casual outings.',
    rating: 4.8,
    price: 32,
    discountPrice: 45,
    image: menJeans,
    category: 'Man',
  },
  {
    id: 24,
    title: 'Kurta',
    description: 'Elegant traditional kurta suitable for festivals, weddings, and cultural occasions.',
    rating: 4.2,
    price: 38,
    discountPrice: 58,
    image: menKurta,
    category: 'Man',
  },
  {
    id: 25,
    title: 'Waistcoat',
    description: 'Premium waistcoat designed to enhance formal and ethnic outfits with a sharp, tailored look.',
    rating: 4.5,
    price: 45,
    discountPrice: 68,
    image: menWaist,
    category: 'Man',
  },
  {
    id: 26,
    title: 'Formal Wear',
    description: 'Smart formal shirt ideal for office wear, meetings, and professional settings.',
    rating: 4.4,
    price: 34,
    discountPrice: 52,
    image: menShirts,
    category: 'Man',
  },
  {
    id: 27,
    title: 'Jersey',
    description: 'Lightweight sports jersey made for comfort, performance, and everyday casual styling.',
    rating: 4.7,
    price: 14,
    discountPrice: 22,
    image: rcbJearsy,
    category: 'Man',
  },
  {
    id: 28,
    title: 'Lower',
    description: 'Comfort-fit lower designed for workouts, lounging, and relaxed daily wear.',
    rating: 4.3,
    price: 27,
    discountPrice: 35,
    image: menLower,
    category: 'Man',
  },
  {
    id: 29,
    title: 'White Sneaker',
    description: 'Trendy white sneakers offering comfort and style for casual outfits and daily use.',
    rating: 4.8,
    price: 36,
    discountPrice: 60,
    image: menSneaker,
    category: 'Man',
  },
  {
    id: 30,
    title: 'Boots',
    description: 'Rugged and durable boots designed for all-day comfort and a bold fashion statement.',
    rating: 4.9,
    price: 48,
    discountPrice: 65,
    image: menBoot,
    category: 'Man',
  },
];


export const womenData = [
  {
    id: 11,
    title: 'Lehenga Choli',
    description: "Wedding-ready lehenga choli with rich embroidery and premium festive fabric.",
    rating: 4.2,
    price: 28,
    discountPrice: 35,
    image: womenLehenga,
    category: 'Women'
  },
  {
    id: 12,
    title: 'Kurti Dupatta',
    description: "Daily wear kurti set with soft fabric and matching dupatta for elegant comfort.",
    rating: 4.1,
    price: 22,
    discountPrice: 27,
    image: womenKurti,
    category: 'Women'
  },
  {
    id: 13,
    title: 'Saree',
    description: "Traditional saree with smooth finish and lightweight fabric for graceful look.",
    rating: 4.3,
    price: 20,
    discountPrice: 30,
    image: womenSarree,
    category: 'Women'
  },
  {
    id: 14,
    title: 'Alloy GreenSilver',
    description: "Elegant designer jewelry set for parties, weddings, and festive looks.",
    rating: 4.6,
    price: 26,
    discountPrice: 37,
    image: womenJewel,
    category: 'Women'
  },
  {
    id: 15,
    title: 'Women Kurta Sharara',
    description: "Flowy kurta sharara set with premium stitching for festive and wedding wear.",
    rating: 4.2,
    price: 30,
    discountPrice: 45,
    image: womenKurtaSharara,
    category: 'Women'
  },
  {
    id: 16,
    title: 'Round Neck Blouse',
    description: "Breathable cotton daily wear panty for soft skin comfort and hygiene.",
    rating: 4.3,
    price: 15,
    discountPrice: 25,
    image: womenBlouse2,
    category: 'Women'
  },
  {
    id: 17,
    title: 'RR SA KURTIS ',
    description: "Comfortable padded bra for daily support and smooth fitting.",
    rating: 4.1,
    price: 18,
    discountPrice: 28,
    image: womenRRSa,
    category: 'Women'
  },
  {
    id: 18,
    title: 'Women Jeans',
    description: "Body-shaping shapewear for tummy control and perfect outfit fitting.",
    rating: 4.8,
    price: 24,
    discountPrice: 29,
    image: womenJeans,
    category: 'Women'
  },
  {
    id: 19,
    title: 'Women Loose Trouser',
    description: "Soft satin babydoll nightwear with lace finish for luxury comfort.",
    rating: 4.5,
    price: 31,
    discountPrice: 43,
    image: womenTrouser,
    category: 'Women'
  },
  {
    id: 20,
    title: 'Women Skirt',
    description: "Stylish flared skirt with smooth fabric for casual and party styling.",
    rating: 4.9,
    price: 21,
    discountPrice: 35,
    image: womenSkirts,
    category: 'Women'
  },
]


export const kidsData = [
  { id: 1, title: 'T-shirt', 
    description: "Soft cotton printed kids t-shirt.",
    rating: 4.5, price: 18,
    discountPrice: 25, 
    image: kidsTshirt, 
    category: 'Kids' 
  },

  { 
    id: 2, 
    title: 'Dhoni Jearsey', 
    description: "Breathable cricket jersey for kids.", 
    rating: 4.0, 
    price: 20, 
    discountPrice: 28, 
    image: kidsJersey, 
    category: 'Kids' },

  { 
    id: 3, 
    title: 'OverSized t-shirt', description: "Oversized trendy kids street wear.", rating: 4.1, price: 22, discountPrice: 30, image: kidsOverSizedTshirt, category: 'Kids' },

  { id: 4, title: 'Kurta', description: "Festive traditional kurta for kids.", rating: 4.2, price: 24, discountPrice: 32, image: kidsEthnic, category: 'Kids' },
  { id: 5, title: 'Kids Casual', description: "Comfortable daily wear casual dress.", rating: 4.8, price: 26, discountPrice: 34, image: kidsCasual, category: 'Kids' },
  { id: 6, title: 'Party Wear', description: "Stylish party wear outfit for kids.", rating: 4.5, price: 28, discountPrice: 36, image: kidsParty, category: 'Kids' },
  { id: 7, title: 'Party Wear', description: "Premium birthday wear for kids.", rating: 4.6, price: 30, discountPrice: 38, image: kidsOtherParty, category: 'Kids' },
  { id: 8, title: 'Sports Sandals', description: "Lightweight kids sandle.", rating: 4.4, price: 32, discountPrice: 40, image: kidsCrocks, category: 'Kids' },
  { id: 9, title: 'Sling Back Flats', description: "Stylish flat clogs for girls.", rating: 4.2, price: 34, discountPrice: 42, image: kidsFootwear, category: 'Kids' },
  { id: 10, title: 'Offroad Monster', description: "High-speed monster toy racing car.", rating: 4.7, price: 38, discountPrice: 45, image: kidsToys, category: 'Kids' }
]
